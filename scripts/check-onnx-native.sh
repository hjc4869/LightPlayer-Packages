...
case "$rid" in
  ...
  win-*)
    ...
    if grep -Ei '(vcruntime|msvcp|libgcc|libstdc|dxcompiler|dxil|webgpu_dawn|onnxruntime_providers).*\.dll' <<< "$dependencies"; then
      printf 'Unbundled Windows runtime dependency.\n' >&2
      exit 1
    fi
    ;;
